 ...
 */
public class ReportProcessor implements ItemProcessor<List<Data>, List<String[]>> {
	...
	public void addData(String id, Object value) {
		...
		if (idExists(id)) {
			...
			throw new RuntimeException("Col id (" + id + ") for data element does not exist...must add column first...");
		}
	}
	...
	private boolean isEmptyReport() {
		/*
		 * if (this.stepExecution.getJobExecution().getExecutionContext().containsKey(EMPTY_REPORT)) { return (Boolean) this.stepExecution.getExecutionContext().get(EMPTY_REPORT); }
		 */
		return false;
	}
